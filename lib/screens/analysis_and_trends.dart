...
import 'dart:io'; 
import '../providers/userdata_provider.dart'; // Assicurati che il percorso sia corretto


// In postman: 
// New request (POST rquest)
...
// We send the reuquest and we have the token (with access and refresh).
...
// I dati devono arrivare dal provider/repository REST
// selectedMetric, selectedPeriod, selectedWindow, possono restare locali
...
class _AnalysisAndTrendsScreenState extends State<AnalysisAndTrendsScreen> {
  ...
  DateTime _shiftByMonths(DateTime base, int months) {
    final int year = base.year + ((base.month - 1 + months) ~/ 12);
    final int month = (base.month - 1 + months) % 12 + 1;
    final int lastDayOfTargetMonth = DateTime(year, month + 1, 0).day;
    final int day = base.day <= lastDayOfTargetMonth
        ? base.day
  ...
  switch (period) {
    case _TrendPeriod.day:
      // Il giorno singolo viene gestito con /day/, non /daterange/
      // start e end coincidono: li usiamo solo come label
      final DateTime baseDay =
          yesterday.add(Duration(days: offset));
      return _DateRange(
    ...
    case _TrendPeriod.week:
      // ✅ max 7 giorni, end = ieri (o ieri ± settimane)
    ...
    case _TrendPeriod.month:
      // ✅ L'API permette max 7 giorni, quindi usiamo finestre da 7
      final DateTime baseEnd =
          yesterday.add(Duration(days: offset * 7));
      final DateTime baseStart =
          baseEnd.subtract(const Duration(days: 6));
 ...
 Future<void> _loadMetricData() async {
  ...
  try {
      ...
      if (_selectedPeriod == _TrendPeriod.day) {
        if (_selectedMetric.apiMetric == 'stress') {
          // Usa il nostro nuovo calcolatore per lo stress (Giorno singolo)
          data = await provider.fetchCalculatedStressSingleDay(range.start);
        } else {
          data = await provider.fetchSingleDayMetric(_selectedMetric.apiMetric, range.start);
        }
      } else {
        if (_selectedMetric.apiMetric == 'stress') {
           // Usa il nostro nuovo calcolatore per lo stress (Range)
           data = await provider.fetchCalculatedStressRange(range.start, range.end);
        } else {
           data = await provider.fetchMetricRange(_selectedMetric.apiMetric, range.start, range.end);
        }
      }
      ...
      // ✅ Fetch the rest of the metrics silently in the background
      ...
      // 1. Prepariamo un messaggio amichevole di default
      String friendlyMessage = 'Oops! We cannot load your data right now.';
      IconData errorIcon = Icons.error_outline;

      // 2. Se l'errore è dovuto alla mancanza di internet, cambiamo il messaggio
      ...
      setState(() {
        ...
        _windowLabel = range.label;
        _errorMessage = friendlyMessage; // Mostra il messaggio nel grafico vuoto
      });

      // 3. Mostriamo lo SnackBar visivo e moderno
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          behavior: SnackBarBehavior.floating, // Lo rende "fluttuante" e staccato dai bordi
  ...
  Future<void> _preloadOtherMetrics() async {
    ...
    for (int i = 0; i < _metricCatalog.length; i++) {
      // Salta la metrica attualmente selezionata, è già stata caricata
      if (i == _selectedMetricIndex) continue; 

      final metric = _metricCatalog[i];
      final String cacheKey = _makeCacheKey(metric.apiMetric, _selectedPeriod, _windowOffset);
      
      // Se abbiamo già i dati in cache per questa finestra di tempo, non facciamo chiamate inutili
      if (_metricCache.containsKey(cacheKey)) continue; 

      // IL BLOCCO DEI DATI FINTI E' STATO RIMOSSO DA QUI.

      // Chiamate API reali e calcoli per TUTTE le metriche, incluso lo stress
      try {
        List<MetricPoint> data;
        
        // Instradamento dinamico: Giorno singolo vs Range
        if (_selectedPeriod == _TrendPeriod.day) {
        ...
        if (mounted) {
          setState(() {
            _metricCache[cacheKey] = const []; // Cache empty list on error
          });
        }
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    final _WindowStats? stats =
      displayPoints.isNotEmpty ? _WindowStats.fromPoints(displayPoints) : null;
    ...
    return Scaffold(
      body: SafeArea(
        child: ListView(
          ...
          children: [
            Text('Analysis & Trends, ${userDataProvider.firstName}', style: textTheme.headlineSmall),
                    ...
                    SizedBox(
                      ...
                      child: isLoading
                          ...
                          : displayPoints.isEmpty
                              ...
                              : ((_selectedMetric.apiMetric == 'sleep' || _selectedMetric.apiMetric == 'stress') &&
            ...
            if (displayPoints.isNotEmpty || _selectedPeriod != _TrendPeriod.day)
              ...[
                          ...
                          children: [
                            ...
                            AnimatedCrossFade(
                              ...
                              secondChild: Padding(
                                            ...
                                            child: Wrap(
                                              ...
                                              children: metricPoints
                                                ...
                                                final bool isHighlighted =
                                                    _highlightedPointIndex == pointIdx;
                                ...
                                crossFadeState:
                                  (isSelected && _selectedPeriod != _TrendPeriod.day)
...
extension on _TrendPeriod {
  String get label {
    switch(this) {
      case _TrendPeriod.day: return 'Day';
      case _TrendPeriod.week: return 'Week';
      case _TrendPeriod.month: return 'Month';
    }
  }
}
...
class _MetricTrendChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Anti-freeze safety check for single-point datasets
    if (points.length == 1) {
      return Center(
        child: Text(
    ...
    return LineChart(
      ...
      LineChartData(
        // Forces the minimum Y axis value to 0 to prevent negative step counts
          ...
          touchCallback: (FlTouchEvent event, LineTouchResponse? touchResponse) {
            if (touchResponse?.lineBarSpots != null && touchResponse!.lineBarSpots!.isNotEmpty) {
              if (event is FlTapUpEvent || event is FlPanUpdateEvent) {
                onPointSelected(touchResponse.lineBarSpots!.first.spotIndex);
              }
            }
          },
          ...
          bottomTitles: AxisTitles(
            sideTitles: SideTitles(
              ...
              getTitlesWidget: (value, meta) {
                ...
                // 1. Mathematical spacing to prevent overlaps
                if (maxLabels != null && maxLabels! > 0) {
                  final int step = (points.length / maxLabels!).ceil().clamp(1, points.length);
                  
                  if (index % step != 0 && index != points.length - 1) {
                    return const SizedBox.shrink();
                  }

                  // Prevents the very last label from overlapping the previous one
                ...
                // 2. Visual cleanup (rounds times like 16:17 to 16:00 for the X axis)
                ...
                return Padding(
                  ...
                  child: Text(
                    ...
                    style: TextStyle(
                      fontSize: 11,
                      color: index == highlightedIndex ? accentColor : textColor,
            ...
            dotData: FlDotData(
              ...
              getDotPainter: (spot, percent, barData, index) {
                ...
                return FlDotCirclePainter(
                  ...
                  strokeColor: isSelected ? Colors.white : textColor.withValues(alpha: 0.35),
                );
              },
            ),
...
class _DailySummaryCard extends StatelessWidget {
  ...
  // Replicata la logica di 'home.dart' per garantire perfetta coerenza cromatica!
  ...
  Widget build(BuildContext context) {
    ...
    // Partiamo dal colore di default passato dal padre
    ...
    if (metricId == 'sleep') {
      ...
      // Override: Se è lo stress, calcoliamo il colore in base al valore e al tema (Chiaro/Scuro)
