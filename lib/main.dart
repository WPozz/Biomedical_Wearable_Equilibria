...
class MyApp extends StatelessWidget { // stateless because the theme and title of the app do not change while the app is in use
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      routes: {
        // Questa rotta funge da "Gatekeeper": decide se andare al MainWrapper o alla LoginPage
        '/auth_gate': (context) => Consumer<AuthProvider>(
          builder: (context, authProvider, child) {
            if (authProvider.isAuthenticated) {
              return const MainWrapper(); // Schermata principale dopo il login
            } else {
              return const LoginPage(); // Schermata di login se non autenticato
            }
          },
        ),
      },
